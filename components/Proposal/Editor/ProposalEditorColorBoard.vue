<template>
  <section class="proposal-editor-color-board">
    <b-button variant="outline-primary" size="sm" block @click="$emit('reset')">
      Reset
    </b-button>
    <div v-for="(row, i) in colorSet" :key="`row-${i}`" class="color-row">
      <div
        v-for="(col, j) in row"
        :key="`col-${j}`"
        class="color-column"
...
<script>
import { highlightColor } from '~/utils/helpers'
