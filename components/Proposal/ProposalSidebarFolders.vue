<template>
  <div class="proposal-sidebar-box">
    <SidebarListBox :loading="loading" :is-empty="folders.length < 1">
      ...
      <template #empty-list>
        ...
        <p>Folders allow you to easily organize your Proposals.</p>
...
import ProposalFolderFormModal from '~/components/Proposal/ProposalFolderFormModal'

import { ALL_PROPOSAL_FOLDERS_QUERY } from '~/graphql/proposal/queries'
import { DELETE_PROPOSAL_FOLDER_MUTATION } from '~/graphql/proposal/mutations'

export default {
  components: { SidebarListBox, ProposalFolderFormModal },
  ...
  methods: {
    addNewList() {
      ...
      this.$bvModal.show('ProposalFolderFormModal')
    },
    ...
    handleRemoveList(folder) {
      ...
      this.$apollo
        .mutate({
          mutation: DELETE_PROPOSAL_FOLDER_MUTATION,
          variables: {
            id: folder.id,
          },
