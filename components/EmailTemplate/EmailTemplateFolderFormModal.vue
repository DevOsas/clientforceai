<template>
  <b-modal
    id="EmailTemplateFolderFormModal"
    size="md"
    modal-class="email-template-folder-form-modal"
    hide-header
    hide-footer
    centered
    @hidden="handleOnHide"
  >
    <template #default="{ hide }">
      <div class="email-template-folder-form-modal-header">
        <h3 class="email-template-folder-form-modal-title">
      ...
      <b-form
        class="email-template-name-form"
        method="POST"
...
<script>
import { SAVE_EMAIL_TEMPLATE_FOLDER_MUTATION } from '~/graphql/email-template/mutations'
...
export default {
  ...
  methods: {
    async saveList(edited) {
      ...
      await this.$apollo
        .mutate({
          mutation: SAVE_EMAIL_TEMPLATE_FOLDER_MUTATION,
          variables: {
            ...this.form.data(),
        ...
        .then(({ data }) => {
          ...
          this.$bvModal.hide('EmailTemplateFolderFormModal')
        })
...
.email-template-folder-form-modal {
  .modal-content {
  ...
  .email-template-folder-form-modal-header {
    margin-bottom: 10px;
    display: flex;
    ...
    .email-template-folder-form-modal-title {
