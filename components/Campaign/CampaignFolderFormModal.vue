<template>
  <b-modal
    id="CampaignFolderFormModal"
    size="md"
    modal-class="campaign-folder-form-modal"
    hide-header
    hide-footer
    centered
    @hidden="handleOnHide"
  >
    <template #default="{ hide }">
      <div class="campaign-folder-form-modal-header">
        <h3 class="campaign-folder-form-modal-title">
...
<script>
import { SAVE_CAMPAIGN_FOLDER_MUTATION } from '~/graphql/campaign/mutations'
...
export default {
  ...
  methods: {
    async saveFolder(edited) {
      this.form.busy = true
      await this.$apollo
        .mutate({
          mutation: SAVE_CAMPAIGN_FOLDER_MUTATION,
          variables: { ...this.form.data(), type: 'campaign' },
        })
        .then(({ data }) => {
          this.form.busy = false
          this.$emit('saved', data.folder)

          this.$bvModal.hide('CampaignFolderFormModal')
        })
...
.campaign-folder-form-modal {
  ...
  .campaign-folder-form-modal-header {
    margin-bottom: 10px;
    display: flex;
    ...
    .campaign-folder-form-modal-title {
